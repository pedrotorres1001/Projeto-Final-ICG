class Game{
	...
	init() {
		...
		this.controller = new WASDController({
			game: this,
			onMove: this.controllerCallback
		});
	}
	...
	controllerCallback(forward, turn) {
...
class WASDController {
    ...
    move() {
        ...
        if (this.keys.w || this.keys.ArrowUp) forward += 0.5;
        if (this.keys.s || this.keys.ArrowDown) forward -= 0.5;
        if (this.keys.a || this.keys.ArrowLeft) turn -= 0.5;
        if (this.keys.d || this.keys.ArrowRight) turn += 0.5;
...
class Preloader{
    ...
    updateTimePassed() {
        ...
        this.timePassedElement.textContent = `Time passed: ${timePassed} ms`;
        setTimeout(() => this.updateTimePassed(), 1000); // update every second
    }
}